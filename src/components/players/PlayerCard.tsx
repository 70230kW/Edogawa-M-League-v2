...
export const PlayerCard: React.FC<PlayerCardProps> = ({ player, standing, rank, currentUserId }) => {
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-4 p-4 border rounded-2xl transition-colors active:scale-[0.98] ${
        isMe
          ? 'bg-accent/10 border-accent/40 hover:border-accent/60'
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {standing ? (
          ...
          <p className="text-xs text-white/25">アカウント未連携</p>
        )}
      </div>
